...
class ShowViewModelFactory(val database: ShowsDatabase, val onShowsLoadCallback: (Unit) -> Unit) : ViewModelProvider.Factory {

    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(ShowViewModel::class.java)) {
            return ShowViewModel(database, onShowsLoadCallback) as T
        }
